...
from main.apps.tutorials.routers import tutorial_submission_events_router
from main.terraform_wars_api import TerraformWarsAPI


root_events_api_router = TerraformWarsAPI(
    title="Terraform Wars Events API",
    urls_namespace="terraform-wars-events-api",
    version="0.0.1",
    description=(
        "Server-sent events API for Terraform Wars application."
        "<br>"
        "<br>"
...
@root_events_api_router.exception_handler(ForbiddenError)
def handle_forbidden_error(request: HttpRequest, exc: ForbiddenError) -> HttpResponse:
    return root_events_api_router.create_response(
        request,
...
@root_events_api_router.exception_handler(NotFoundError)
...
root_events_api_router.add_router("/notifications", notification_events_router, tags=["notification_events"])
root_events_api_router.add_router("/submissions", tutorial_submission_events_router, tags=["submission_events"])
