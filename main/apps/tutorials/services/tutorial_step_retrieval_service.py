...
class TutorialStepRetrievalService:
    ...
    def _get_tutorial_step_for_read_by_slug(self, tutorial_slug: str, tutorial_step_slug: str) -> TutorialStep:
    ...
    def _get_tutorial_step_for_read_by_id(self, tutorial_step_id: UUID) -> TutorialStep:
        return self._get_tutorial_step_queryset(
            select_related_fields=["tutorial", "tutorial__provider", "tutorial__author"],
            prefetch_related_fields=[models.Prefetch("tutorial__tags")],
    ...
    def get_tutorial_step_detail_by_slug(self, tutorial_slug: str, tutorial_step_slug: str) -> TutorialStep:
        try:
            return self._get_tutorial_step_for_read_by_slug(tutorial_slug, tutorial_step_slug)
        except TutorialStep.DoesNotExist:
            raise NotFoundError(_("Tutorial step not found"))

    def get_tutorial_step_detail_by_id(self, tutorial_step_id: UUID) -> TutorialStep:
        try:
            return self._get_tutorial_step_for_read_by_id(tutorial_step_id)
        except TutorialStep.DoesNotExist:
            raise NotFoundError(_("Tutorial step not found"))
